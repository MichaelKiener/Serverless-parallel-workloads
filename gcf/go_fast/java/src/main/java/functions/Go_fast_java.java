...
import gofastpackage.*;

public class Go_fast_java implements HttpFunction {
  private final AtomicInteger count = new AtomicInteger(0);

  private static final Logger logger = Logger.getLogger(Go_fast_java.class.getName());
  ...
  public void service(HttpRequest request, HttpResponse response)
      throws IOException {
        ...
        for (int i = 0; i < executionIDs.size(); i++) {
          System.out.println(executionIDs.get(i));
    ...
    try {
      results = gf.runGoFast(input, threads);
    } catch (Exception e) {
      results = "Error";
    }
    results += "\n" + addInfo;
