...
public class Monte_java implements HttpFunction {
  private final AtomicInteger count = new AtomicInteger(0);

  private static final Logger logger = Logger.getLogger(Monte_java.class.getName());
  ...
  public void service(HttpRequest request, HttpResponse response)
    ...
    try {
      results = monte.runMonte(input, threads);
    } catch (Exception e) {
      results = "Error";
    }
    results += "\n" + addInfo;
